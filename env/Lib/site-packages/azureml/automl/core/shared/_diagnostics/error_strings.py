...
class AutoMLErrorStrings:
    ...
    HIERARCHY_QUANTILE_FORECAST_AGGREGATION_UNSUPPORTED = "Quantile forecasts are not currently supported when " \
                                                          "the forecast level contains aggregates from training " \
                                                          "level nodes. Valid values for {forecast_level_param} " \
                                                          "are: {valid_forecast_levels}."
    HIERARCHY_QUANTILE_FORECAST_INPUT_EXTRA_ARGS = "Invalid option in arguments: {quantile_option}. " \
                                                   "Please specify forecast_quantiles in the {class_name} constructor."
    ...
    NLP_INCORRECT_CLASSIFICATION_DATASET = "Incorrect input data type is used for {split_type:log_safe} set file. " \
                                           "Classification scenarios expect TabularDataset format. " \
                                           "More information can be found at {info_link:log_safe}."
    ...
    NLP_MIXED_MULTICLASS_TYPES = "Expected all labels to be of type integer, boolean, string, or their numpy " \
                                 "equivalents, but found type {bad_type:log_safe}."
    ...
    POWER_TRANSFORMER_INVERSE_TRANSFORM = "Failed to inverse transform: y_min is greater than the observed minimum " \
                                          "in y."
    QUANTILE_FORECAST_ROLLING_MODE_UNSUPPORTED = 'forecast_quantiles does not support rolling forecast mode.' \
                                                 'Please use recursive forecast mode with forecast_quantiles'
    ...
    TCN_DEADLOCK_ERROR = "TCN run has met deadlock error."
    ...
    UNIQUE_VALUE_CROSS_VALIDATION = "The cross validation fold contains only unique target values in " \
                                    "the training portion. It usually means that the training set contains " \
                                    "mostly the same values for each time series. Please decrease forecast_horizon, " \
                                    "cv_step_size and n_cross_validations and train the model again."
