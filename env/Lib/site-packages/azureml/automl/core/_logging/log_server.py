...
def enable_telemetry(key: Optional[str],
                     component_name: Optional[str] = logging_fields.TELEMETRY_AUTOML_COMPONENT_KEY) -> None:
    """
    ...
    :param component_name: The component name to get telemetry log handler. Default, automl component key
    :return:
    """
    if not key:
        if not DEBUG_MODE:
        ...
        for telemetry_key in key.split(","):
            handler = get_telemetry_log_handler(
                instrumentation_key=telemetry_key, component_name=component_name
            )
