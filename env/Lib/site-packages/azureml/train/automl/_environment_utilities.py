...
def modify_run_configuration(settings: AzureAutoMLSettings,
    ...
    required_package_list = [
        ...
        _Package(numpy_pkg, numpy_regex, True, "==1.21.6"),
        ...
        _Package(boto3_pkg, boto3_regex, False, "==1.15.18"),
        _Package(botocore_pkg, botocore_regex, False, "==1.18.18"),
    ...
    dependencies.set_python_version("3.8.16")
...
_AUTOML_DOCKERFILE_GRAMMAR = \
    re.compile(r'(.*)'          # 0 section0
               '(^\s*# begin conda create\s*$)'           # 1 begin conda create marker
               '(.*)'                                     # 2 begin_conda create
               '(^\s*# begin conda dependencies\s*$)'     # 3 begin conda dependencies marker
               '(.*)'                                     # 4 conda dependencies
               '(^\s*# end conda dependencies\s*$)'       # 5 end conda dependencies marker
               '(.*)'                                     # 6 end_conda_create
               '(^\s*# end conda create\s*$)'             # 7 end conda create marker
               '(.*)'                                     # 8 section1
               '(^\s*# begin pip install\s*$)'            # 9 begin pip install marker
               '(.*)'                                     # 10 begin_pip_install
               '(^\s*# begin pypi dependencies\s*$)'      # 11 begin pypi dependencies marker
               '(.*)'                                     # 12 pypi dependencies
               '(^\s*# end pypi dependencies\s*$)'        # 13 end pypi dependencies marker
               '(.*)'                                     # 14 end_pip_install
               '(^\s*# end pip install\s*$)'              # 15 end pip install marker
               '(.*)',                                    # 16 section2
