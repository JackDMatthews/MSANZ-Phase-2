...
class DataSourceDef(object):
    ...
    def calculate_hash(self):
        ...
        # Append sql related fields
        sql_fields = [f for f in [
            self.sql_query, self.sql_table,
            self.sql_stored_procedure] if f]
        if sql_fields:
            values.extend(sql_fields)
        if self.sql_stored_procedure_params:
            values.extend([param._serialize_to_string() for param in self.sql_stored_procedure_params])
...
class StoredProcedureParameter(object):
    ...
    def _serialize_to_string(self):
        """Serialize param to string for datasource hash calculation."""
